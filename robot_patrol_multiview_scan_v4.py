...
# OpenCV VideoCapture. Multiview scan images are therefore captured by
# reopening the stream, allowing pan/tilt to settle, and flushing frames.
CAMERA_SETTLE_SEC = 0.45
SCAN_FLUSH_FRAMES = 10

# Scan/person-detection controls.
...
AUTO_PAUSE_AFTER_SCAN_AND_DETECT = True
STOP_AFTER_FINAL_SURVEY_SCAN = False   # keep patrolling after final survey scan
...
# Stall guards: if a turn command succeeds but yaw does not change, the robot
# is probably physically stuck in carpet or against an obstacle.
...
class CameraClient:
    ...
    def flush(self, n: int = SCAN_FLUSH_FRAMES) -> int:
    ...
    def capture_fresh(self, settle_sec: float = CAMERA_SETTLE_SEC, flush_frames: int = SCAN_FLUSH_FRAMES, reopen_stream: bool = False):
    ...
    def capture_scan_frame(self):
        """Fresh still frame for multiview scan after pan/tilt movement."""
        return self.capture_fresh(
            settle_sec=CAMERA_SETTLE_SEC,
            flush_frames=SCAN_FLUSH_FRAMES,
            reopen_stream=True,
        )
...
def escape_search_one_direction(search_dir: str) -> bool:
    ...
    for step in range(1, MAX_SWEEP_STEPS + 1):
        ...
        yaw_delta = abs(angle_diff_deg(yaw, yaw_before_turn))
        ...
        if yaw_no_change_count >= YAW_NO_CHANGE_LIMIT:
            ...
            reverse_steps(n=REVERSE_STEPS_HARD, duration=0.45)
            time.sleep(0.15)
...
def smart_escape(last_turn: str) -> str:
    ...
    print("[ESCAPE] both directions failed")
...
def scan_and_detect(camera: CameraClient, detector: PersonDetector, save_segmented: bool = False):
    ...
    for label, pan_fn, tilt_fn in positions:
        ...
        fresh_frame = camera.capture_scan_frame()
...
def patrol():
    ...
    print("Starting robot_patrol_multiview_scan_v4: fast IMU patrol + gated multiview scan...")
    ...
    try:
        ...
        while True:
            ...
            scan_space_ok = (
                    before_dist >= OPEN_SPACE_SCAN_MIN_CM
                    and after_dist >= OPEN_SPACE_SCAN_MIN_CM
                    and after_dist < 280
            )

            if force_exit_scan and not scan_space_ok:
                print(
                    f"[SCAN] final scan pending but not enough open space "
                    f"(before={before_dist}, after={after_dist})"
                )

            should_run_yolo_scan = scan_space_ok and (
                    force_exit_scan or (
                        enough_point_gap
            ...
            if should_run_yolo_scan:
                ...
                if AUTO_PAUSE_AFTER_SCAN_AND_DETECT:
                    pause_reason = "person detected during scan" if detected else "after multiview scan"
                    if not pause_until_resume(pause_reason):
                        break
